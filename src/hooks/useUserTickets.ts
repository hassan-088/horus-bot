...
function toUserTicket(
  ...
  const baseTicket = {
    ...
    tour_duration: ((robotTourTicket?.duration_minutes ?? robotTourTicket?.tour_duration_min) as number) ?? null,
    pace: (robotTourTicket?.pace as string) ?? null,
    interests: asStringArray(robotTourTicket?.selected_interests ?? robotTourTicket?.interests),
    selected_exhibits: asStringArray(robotTourTicket?.selected_artifact_ids ?? robotTourTicket?.selected_exhibits),
    accessibility: asStringArray(robotTourTicket?.accessibility_preferences ?? robotTourTicket?.accessibility),
    preferred_language: ((robotTourTicket?.language ?? robotTourTicket?.preferred_language) as string) ?? null,
    preferred_language_other: (robotTourTicket?.preferred_language_other as string) ?? null,
    photo_spots: ((robotTourTicket?.include_photo_stops ?? robotTourTicket?.photo_spots) as boolean) ?? null,
      ...
      try {
        ...
        const created = toUserTicket(
          ...
          },
          {
            ...
            duration_minutes: input.tour_duration_min ?? null,
            language: input.preferred_language ?? null,
            preferred_language_other: input.preferred_language_other ?? null,
            selected_artifact_ids: input.selected_exhibits ?? [],
            selected_interests: [],
            accessibility_preferences: [],
            include_photo_stops: input.photo_spots ?? false,
